...
class CrossModalGatedAttention(nn.Module):
  ...
  def forward(self, x_1, x_2):
    ...
    if B_1 != B_2:
      sys.stderr.write("batch size does not match for two modalities")
    else:
      batch_size = B_1
    if N_1 != N_2:
      sys.stderr.write("the number of frames does not match for two modalities")
    else:
      num_frames = N_1
    if F_1 != F_2:
      sys.stderr.write("the number of features does not match for two modalities")
    else:
      feature_size = F_1
    ...
    qkv = self.qkv(z_i)# (B x N x 3D)
    qkv = qkv.reshape(batch_size, num_frames, 3, D) # (B x N x 3 x D)
    _, k_i, v_i = qkv.permute(2, 0, 1, 3) # each: (B x N x D)
    q_j, _, _ = qkv.permute(2, 0, 1, 3) # each: (B x N x D)
    ...
    Q, K, V = QKV.permute(2, 0, 3, 1, 4) # each: (B x N x Nh x Dh)
    Attn = F.softmax(torch.matmul(Q, K.transpose(-2, -1)) * self.Scale, dim=-1) # (B x N x Nh x Nh) ## it would be (B x Nh x N x N)  !??
    H = self.Att_drop(Attn) @ V # (B x N x Nh x Dh)
    H = H.reshape(batch_size, num_frames, -1) # (B x N x D) : Nh x Dh -> D
